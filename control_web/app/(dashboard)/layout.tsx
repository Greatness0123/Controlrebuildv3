...
import {
  ...
  Link as LinkIcon, ChevronLeft, ChevronRight, Loader2, Menu, X
} from 'lucide-react';
...
export default function WorkspaceLayout({ children }: { children: ReactNode }) {
    ...
    <>
      ...
      <div className="h-14 flex items-center justify-between px-3 border-b border-white/5">
        {(sidebarOpen || mobileOpen) && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold tracking-tighter">CONTROL <span className="text-zinc-500">WORKSPACE</span></span>
          </div>
        )}
      ...
      <div className="p-2">
        ...
        >
          ...
          {(sidebarOpen || mobileOpen) && (isCreatingChat ? 'Creating...' : 'New Chat')}
        </button>
      </div>
  ...
  return (
    <div className="h-screen bg-black text-white flex overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-72 flex flex-col border-r border-white/5 bg-zinc-950 transition-transform duration-250 ease-in-out md:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
