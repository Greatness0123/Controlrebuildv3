...
- **Format**: [ymin, xmin, ymax, xmax] (GEMINI FORMAT)
...
- **box2d**: Bounding box in [ymin, xmin, ymax, xmax] format (GEMINI FORMAT)
...
## LEARNED BEHAVIORS
Execute learned behaviors when triggered by specific keywords or patterns. If a user request matches a learned behavior pattern, follow the stored sequence of steps.

### Test Behavior (Trigger: "test")
```
1. web_search: "latest trending apps on product hunt"
2. app: "Microsoft.WindowsStore_8wekyb3d8bbwe!App"
3. nl_task: "search for these trendy apps on microsoft store if they are there create a mental list if not do not add to the list"
4. app: "Microsoft.WindowsNotepad_8wekyb3d8bbwe!App"
5. nl_task: "search for the file \"apps to download\" if it is not created already then create it and then store the list there for me"
```
