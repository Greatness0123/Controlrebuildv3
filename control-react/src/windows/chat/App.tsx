...
export default function ChatApp() {
  ...
  const handleSend = () => {
    ...
    if (inputValue.startsWith('/')) {
        const cmd = SLASH_COMMANDS.find(s => s.command === inputValue.trim());
        if (cmd) {
            // Handle command
            setInputValue('');
    ...
    executeTask(inputValue, mode);
    setInputValue('');
  };

  return (
    <div className="flex flex-col h-screen bg-bg-base text-text-primary overflow-hidden border-l border-border-strong shadow-2xl">
        ...
        <div className="flex items-center gap-1" style={{ WebkitAppRegion: 'no-drag' } as any}>
          ...
          <Tooltip content={isWakewordEnabled ? "Mute Wakeword" : "Enable Wakeword"}>
            <button onClick={() => toggleWakeword(!isWakewordEnabled)} className={`p-2 rounded-md transition-colors ${isWakewordEnabled ? 'text-text-primary' : 'text-text-disabled hover:bg-white/5'}`}>
              <Icon name={isWakewordEnabled ? 'Mic' : 'MicOff'} size="md" />
            </button>
          </Tooltip>
      ...
      <footer className="p-4 space-y-4 bg-bg-base/80 backdrop-blur-md border-t border-border-subtle relative">
        <AnimatePresence>
            {showSlashCommands && (
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 20 }}
                    className="absolute bottom-full left-4 right-4 mb-2 bg-bg-elevated border border-border-strong rounded-xl shadow-2xl overflow-hidden overflow-y-auto max-h-48 z-50"
                >
                    {SLASH_COMMANDS.map((s, i) => (
                        <button
                            key={i}
                            onClick={() => { setInputValue(s.command + ' '); setShowSlashCommands(false); inputRef.current?.focus(); }}
                        ...
                        >
                            <span className="font-mono text-xs text-white">{s.command}</span>
        ...
        <div className="flex items-end gap-2 p-2 bg-bg-surface border border-border-default focus-within:border-border-strong rounded-2xl transition-colors">
          <button className="p-2 rounded-lg hover:bg-white/5 text-text-muted hover:text-white transition-colors"><Icon name="Paperclip" size="md" /></button>
          <textarea
            ref={inputRef}
            rows={1}
            value={inputValue}
            onChange={(e) => { setInputValue(e.target.value); setShowSlashCommands(e.target.value === '/'); }}
