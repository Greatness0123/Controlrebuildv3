...
class ComputerUseAgent {
  ...
  async init() {
    try {
      ...
      this.edgeTTS.on("speaking", () => {
        ...
        const chatWin2 = this.windowManager.getWindow("chat");
        if (chatWin2 && !chatWin2.isDestroyed()) {
          chatWin2.webContents.send("audio-started", {});
        }
      });
      ...
      this.edgeTTS.on("queue-empty", () => {
        ...
        const chatWin2 = this.windowManager.getWindow("chat");
        if (chatWin2 && !chatWin2.isDestroyed()) {
          chatWin2.webContents.send("audio-stopped", { queueEmpty: true });
        }
      });
  ...
  setupIPCHandlers() {
    ...
    ipcMain.handle("set-picovoice-key", async (event, key) => {
      try {
        ...
        dbService.updateUser(this.currentUser.id, { picovoiceKey: key }).then((res) => console.log("[Main] Picovoice key updated in DB:", res.success)).catch((e) => console.warn("[Main] DB key update failed, using local only:", e.message));
          ...
          for (const filePath of result.filePaths) {
            try {
              ...
              if (ext === ".json") {
                ...
                const name = path.basename(filePath, ext).replace(/[^a-zA-Z0-9]/g, " ").split(" ").map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join("");
                const skill = {
                  name,
    ...
    ipcMain.handle("update-skill", async (event, payload) => {
      ...
      return { success: ok, error: ok ? void 0 : "Could not update (duplicate name or not found)" };
    });
    ...
    ipcMain.handle("execute-task", async (event, task, mode) => {
      ...
      let apiKey = this.appSettings.geminiApiKey;
      if (!apiKey) {
    ...
    ipcMain.handle("show-prompt-modal", async (event, message, defaultValue, options) => {
      ...
      if (chatWin && !chatWin.isDestroyed()) {
        ...
        return new Promise((resolve) => {
          ipcMain.once(`prompt-response:${requestId}`, (event2, value) => {
            resolve(value);
          });
        });
      }
  ...
  getSettings() {
    ...
    settings.windowVisibility = this.appSettings.windowVisibility !== void 0 ? this.appSettings.windowVisibility : true;
    ...
    settings.ttsRate = this.appSettings.ttsRate !== void 0 ? this.appSettings.ttsRate : 1;
    settings.ttsVolume = this.appSettings.ttsVolume !== void 0 ? this.appSettings.ttsVolume : 1;
    const cachedKeys = dbService.getKeys();
...
`;
  ...
  async saveSettings(settings) {
    ...
    try {
      ...
      if (settings.layout !== void 0) {
        this.appSettings.layout = settings.layout;
      }
      if (settings.modelProvider !== void 0) {
      ...
      if (settings.universalApiKey !== void 0) this.appSettings.universalApiKey = settings.universalApiKey;
      if (settings.universalModel !== void 0) this.appSettings.universalModel = settings.universalModel;
      if (settings.universalBaseUrl !== void 0) this.appSettings.universalBaseUrl = settings.universalBaseUrl;
      if (settings.cloudRegion !== void 0) this.appSettings.cloudRegion = settings.cloudRegion;
      if (settings.cloudCredentials !== void 0) this.appSettings.cloudCredentials = settings.cloudCredentials;
      if (settings.cloudModel !== void 0) this.appSettings.cloudModel = settings.cloudModel;
      if (settings.openaiApiKey !== void 0) this.appSettings.openaiApiKey = settings.openaiApiKey;
      if (settings.openaiModel !== void 0) this.appSettings.openaiModel = settings.openaiModel;
      if (settings.anthropicApiKey !== void 0) this.appSettings.anthropicApiKey = settings.anthropicApiKey;
      if (settings.anthropicModel !== void 0) this.appSettings.anthropicModel = settings.anthropicModel;
      if (settings.xaiApiKey !== void 0) this.appSettings.xaiApiKey = settings.xaiApiKey;
      if (settings.xaiModel !== void 0) this.appSettings.xaiModel = settings.xaiModel;
      if (settings.deepseekApiKey !== void 0) this.appSettings.deepseekApiKey = settings.deepseekApiKey;
      if (settings.deepseekModel !== void 0) this.appSettings.deepseekModel = settings.deepseekModel;
      if (settings.moonshotApiKey !== void 0) this.appSettings.moonshotApiKey = settings.moonshotApiKey;
      if (settings.moonshotModel !== void 0) this.appSettings.moonshotModel = settings.moonshotModel;
      if (settings.zaiApiKey !== void 0) this.appSettings.zaiApiKey = settings.zaiApiKey;
      if (settings.zaiModel !== void 0) this.appSettings.zaiModel = settings.zaiModel;
      if (settings.litellmApiKey !== void 0) this.appSettings.litellmApiKey = settings.litellmApiKey;
      if (settings.litellmModel !== void 0) this.appSettings.litellmModel = settings.litellmModel;
      if (settings.minimaxApiKey !== void 0) this.appSettings.minimaxApiKey = settings.minimaxApiKey;
      if (settings.minimaxModel !== void 0) this.appSettings.minimaxModel = settings.minimaxModel;
      if (settings.lmstudioApiKey !== void 0) this.appSettings.lmstudioApiKey = settings.lmstudioApiKey;
      if (settings.lmstudioModel !== void 0) this.appSettings.lmstudioModel = settings.lmstudioModel;
      if (settings.ttsVoice !== void 0) {
        this.appSettings.ttsVoice = settings.ttsVoice;
        this.edgeTTS.setVoice(settings.ttsVoice);
      }
      if (settings.ttsRate !== void 0) {
        this.appSettings.ttsRate = settings.ttsRate;
        this.edgeTTS.setRate(settings.ttsRate);
      }
      if (settings.ttsVolume !== void 0) {
  ...
  cleanMarkdownForTTS(text) {
    if (!text) return "";
    return text.replace(/\*\*(.*?)\*\*/g, "$1").replace(/\*([^*\n]+)\*/g, "$1").replace(/__([^_]+)__/g, "$1").replace(/`([^`]+)`/g, "$1").replace(/```[\s\S]*?```/g, "Code block skipped").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").replace(/^#+\s+/gm, "").replace(/^\s*[-*+]\s+/gm, "").replace(/[*_~`]/g, "").trim();
  }
}
